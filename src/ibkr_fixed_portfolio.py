...
class StockList:
    ...
    def _set_stock_lists(self):
        ...
        self.watchlist_positions = self._util.read_symbols(self._util.get_latest_watchlist_file(pattern='ibkr_watchlist'))
        self._scanner_positions = []
        if self._leverage > 0:
            self._scanner_positions: list[ScannerPosition] = self._sc.scan_list(stock_list=self.watchlist_positions, flag_is_long=FLAG_IS_LONG)
